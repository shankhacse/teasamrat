...
if(mode=="Edit")
{
repopulatePurchaseBill(basepath); 
enableVendor();
}
...
//txtamounttobecredited
 ...
 $('.payment').keypress(function(event){
   ...
   $('.add').click(function(){
      ...
      var row="<tr>"+
              "<td>"+bill+"<input type='hidden' name='vendorBillMstId' value='"+vendorBillMasterId+"'/><input type='hidden' name='vendBillMasterIDS[]' value='"+vendorBillMasterId+"'/></td>"+
              "<td style='text-align: center'>"+billDate+"</td>"+
              "<td style='text-align:right'>"+amount+"</td>"+
              "<td style='text-align:right'>"+adjusted+"</td>"+
              "<td style='text-align:right'>"+
              "<img src='"+basepath+"application/assets/images/delete-ab.png"+"' data-vendorbillmastid='"+vendorBillMasterId+"' alt='del' class='rowDel' style='cursor: pointer;' /></td>"
              +"</tr>";
        if(rowValidation())
        {
   ...
   $("#billAdjustTable").on('click', '.rowDel', function () {
		...
		$(this).closest('tr').remove();
			if(mode=="Edit"){
				...
				if(conf)
				{
					var vendorbillmasterID = $(this).data('vendorbillmastid');
					var vendorpaymentdtlId = $(this).data('vendorpaymentdtlid');
					console.log(vendorpaymentdtlId);
					deleteDetailData(basepath,vendorpaymentdtlId,vendorbillmasterID,'VPAYMENT');
				}
			}
		repopulatePurchaseBill(basepath);
        enableVendor();
...
$("#saveVendorPayment").click(function(){
   ...
   var vendorPaymentId = $("#vendorPaymentId").val()||0;
    if(validation()){
      $("#saveVendorPayment").css("display","none");
        $.ajax({
                type: 'POST',
                ...
                success: function (data) {
                    ...
                    if (data.msg==1) {
                     ...
                     $("#saveVendorPayment").css("display","block");
                }
            });
...
function deleteDetailData(basepath,vendorpaymentdtlId,vendorbillmasterID,from)
{
    $.ajax({
            url: basepath + 'vendorpayment/deleteDetailData',
            data: {vendorpaymentdtlId:vendorpaymentdtlId,vendorbillmasterID:vendorbillmasterID,from:from},
            type: "POST",
            dataType: "json",
            success: function (data) {
...
//repopulate purchasebillList
function repopulatePurchaseBill(basepath)
{
  //alert(basepath);
   $("#billList").html('<img src="'+basepath+'application/assets/images/verify_logo.gif" alt=""> <b>Please Wait...</b>');

      var formDataserialize = $("#vendorPaymentForm" ).serialize();
      formDataserialize = decodeURI(formDataserialize);
      console.log(formDataserialize);
      var formData = {formDatas: formDataserialize};
      $.ajax({
            url: basepath + 'vendorpayment/rePopulatePurchaseBillList',
            data: formData,
            type: "POST",
            dataType: "html",
            success: function (data) {
...
{
var dtlLength = $('input[name="vendBillMasterIDS[]"]').length;
if(dtlLength>0)
{
