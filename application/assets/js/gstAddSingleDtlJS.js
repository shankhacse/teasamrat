...
$(document).ready(function(){
	...
	$(document).on("submit",'#purchaseInvoiceDetailSingle',function(e){
		e.preventDefault();
		
		
		if(validateSingleDtl())
		{
			$("#singledtlerror").css("display","none");
			var formData = $("#purchaseInvoiceDetailSingle" ).serialize();
			formData = decodeURI(formData);
			console.log(formData);
			
			
			$("#addinvocesingle").css('pointer-events','none');
			
			$.ajax({
					type: "POST",
					url: basepath+'gstpurchaseinvoice/addSinglePurchaseInvoice',
					dataType: "json",
					...
					success: function (result) {
						...
						{
							alert("Message : "+result.msg_data);
							$("#addinvocesingle").css('pointer-events','');
							$("#purchaseInvoiceDetailSingle")[0].reset();
							$("#addinvocesingle").css('display','none');
						}
						else
						{
							alert("Error : Some problem please try again.");
						}
					}, 
...
function validateSingleDtl()
{
	var lotno = $("#txtLot_999").val();
	var inv = $("#txtInvoice_999").val();
	var price = $("#txtPrice_999").val();
	var group = $("#drpGroup_999").val();

	var garden = $("#drpGarden_999").val();
	var grade = $("#drpGrade_999").val();
	var normalbag = $("#txtNumOfNormalBag_999").val();
	var normalbagqty = $("#txtNumOfNormalNet_999").val();
	var cgst = $("#cgst_0_999").val();
	var sgst = $("#sgst_0_999").val();
	var igst = $("#igst_0_999").val();
	
	$("#singledtlerror").css("display","block");
	$("#singledtlerror").text();
	if(lotno=="")
	{
		$("#singledtlerror").text("Error : Enter Lot No");
		return false;
	}
	if(inv=="")
	{
		$("#singledtlerror").text("Error : Enter Invoice No");
		return false;
	}
	if(price=="" || price=="0")
	{
		$("#singledtlerror").text("Error : Enter Price");
		return false;
	}
	if(group=="0")
	{
		$("#singledtlerror").text("Error : Select Group");
		return false;
	}
	if(garden=="0")
	{
		$("#singledtlerror").text("Error : Select Garden");
		return false;
	}
	if(grade=="0")
	{
		$("#singledtlerror").text("Error : Select Grade");
		return false;
	}
	if(normalbag=="" || normalbag=="0")
	{
		$("#singledtlerror").text("Error : Enter no of normal bag");
		return false;
	}
	
	if(normalbagqty=="" || normalbagqty=="0")
	{
		$("#singledtlerror").text("Error : Enter no qty of normal bag");
		return false;
	}
