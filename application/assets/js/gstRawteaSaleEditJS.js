...
$(document).ready(function() {
    ...
    var invoice_no = $("#invoice_no").val();
     ...
     $.ajax({
            url :  basepath + "gstrawteasale/checkExistingRawTeaInvoiceNo",
            type: "POST",
            dataType:'json',
...
*/
    ...
    $("#dropdown-garden").change(function() {
        ...
        $("#imgInvoice").show();
        $.ajax({
            url: basepath + "gstrawteasale/showInvoice",
            type: 'post',
...
//invoice dropdown change
    $(document).on('change', "#dropdown-invoice", function() {
        ...
        $("#imgLot").show();
        $.ajax({
            url: basepath + "gstrawteasale/showLotNumber",
            type: 'post',
...
//lot dropdown change
    $(document).on('change', "#dropdown-lot", function() {
        ...
        $("#imgGrade").show();
        $.ajax({
            url: basepath + "gstrawteasale/showGrade",
            type: 'post',
...
//lot dropdown change
 ...
 $(document).on('blur', ".usedBag", function() {
        ...
        var cgstID = $("#cgst_"+dtl_id).val();
        var sgstID = $("#sgst_"+dtl_id).val();
        var igstID = $("#igst_"+dtl_id).val();
        ...
        if (getValidation(dtl_id)) 
        {
            ...
            var cgst_amt=getGSTTaxAmount(cgstID,dtl_id,"CGST");
            var sgst_amt=getGSTTaxAmount(sgstID,dtl_id,"SGST");
            var igst_amt=getGSTTaxAmount(igstID,dtl_id,"IGST");

            $("#cgstAmt_"+dtl_id).val(cgst_amt);
            $("#sgstAmt_"+dtl_id).val(sgst_amt);
            $("#igstAmt_"+dtl_id).val(igst_amt);
...
function taxableamount(dtlId){
    ...
    var delivercharge = parseFloat($("#txtDelivery_"+dtlId).val()||0); 
    var amount = parseFloat($("#txtBlendedPrice_"+dtlId).val()||0);
    
    totalItemAmount = (amount+delivercharge)-discountamount;
...
function detailValidationRow()
{
    ...
    $(".usedBag").removeClass("custom_err");
    $(".rate").removeClass("custom_err");
    ...
    $('input[name^="txtused"]').each(function() {
      ...
      // SaleBag Validation ---------------------------------------------
      var salebagID = "#txtused_"+idsplit1[1];
      var salebag = parseFloat(($(salebagID).val() == ""||0.00? "" : $(salebagID).val()));
      if(isNaN(salebag)) {salebag = 0;}
     
    if(salebag>0)
    {
        var rateID = "#txtrate_"+idsplit1[1];
        var rate = parseFloat(($(rateID).val() == ""||0.00? "" : $(rateID).val()));
        if(isNaN(rate)) {rate = 0;}
       
        if(rate<=0)
        {
            $(rateID).addClass("custom_err");
            isValid =  false;
        }

        // GST Validation ---------------------------------------------
        var cgst = "#cgst_"+idsplit1[1];
        var sgst = "#sgst_"+idsplit1[1];
        var igst = "#igst_"+idsplit1[1];
...
$(document).on('click','.del-row',function(){
	var sale_dtl_id = $(this).attr('id');
	console.log(sale_dtl_id);
	 if (confirm("Do you want to delete ?")) {
	 $('#rowdtl_' + sale_dtl_id).remove();
	 callAllTotalFunction();
	 }
});
...
function getConsignee(val) {
 ...
 var basepath = $("#basepath").val();
  ...
  $.ajax({
  type: "POST",
  url: basepath+'gstrawteasale/getConsignee',
  data: {customer_id:val},
  
  success: function(data){
    $("#sel_consignee").html(data);
  },
