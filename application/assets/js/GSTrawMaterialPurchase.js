 ...
 $("#vendor").customselect();
   ...
   $("#saveRawMaterial").click(function(){
      ...
      if(getValidation()){
               ...
               if(data==1){
                    ...
                    $("#rawmaterial_save_dialg").dialog({
                                                 modal: true,
                                                    buttons: {
                                                Ok: function() {
                                                     window.location.href = basepath + 'rawmaterialpurchase/gstRawmaterialPurchase';
                                                    $( this ).dialog( "close" );
         ...
         }//end if check incoice existance
    ...
    $(document).on('blur', ".pacQty", function() {
    ...
    $(document).on('change', ".productlist", function() {
      ...
      //  var selectorId = $(".discamount").attr("id");
        //alert(discounamt);
        ...
        $.ajax({
            ...
            success: function(data)
            {
               $(txtRateId).val(data.purchase_rate);     //data - response from server
               $(txtHSNId).val(data.hsncode);     //data - response from server
                // added on 02.05.2018 

                getAmount(master_id, detail_id);
                getTaxableamount(discounamt,id); 
                resetGstTax(detailIdarr[1],detailIdarr[2]);
                callAllTotalFunction();
            },
            error: function (e)
            {
...
$(document).on('keyup', '.rate', function () {
        ...
        var master_id = arr[1]; //added 03.05.2018
        var detail_id = arr[2]; //added 03.05.2018
...
$(document).on('blur', '.pacQty', function () {
      ...
      getTaxableamount(discounamt,selectorid);
     callAllTotalFunction();
     resetGstTax(arr[1],arr[2]);
     //alert(discounamt);
    });
...
function getValidation(){
    ...
    var txtInvoiceValue = $("#txtInvoiceValue").val();
    $(".invoice-validation-err").text('');
   if(invoiceNo==""){
   ...
   if(vendor==0){
    ...
    $("#txtTotalAmount").addClass("glowing-border");
     $(".invoice-validation-err").text("Add Tax Details !");
         return false;
   }
   if(txtInvoiceValue=="" || txtInvoiceValue=="0.00"){
    $("#txtInvoiceValue").addClass("glowing-border");
     $(".invoice-validation-err").text("Add Tax Details !");
         return false;
   }
...
// check invoice against vendor

function checkPurchaseInvoiceExist(){

  
    var basepath = $("#basepath").val();
    var oldinvoiceno = $("#oldinvoiceno").val();
    var invoiceno = $("#invoiceno").val();
    var oldvendor = $("#oldvendor").val();
    var vendor = $("#vendor").val();
   /* alert(oldinvoiceno);
    alert(invoiceno);
    alert(oldvendor);
   ...
   var isValid=true;
     $(".invoice-validation-err").text('');
   if (oldinvoiceno!=invoiceno || oldvendor!=vendor) {
        $.ajax({
            type: "POST",
            url: basepath + "rawmaterialpurchase/checkInvoiceExistance",
            data: {invoiceno:invoiceno,vendor:vendor},
            dataType: 'html',
            success: function(data) {
               if(data==1){
               //alert("Already exist");
               isValid=false;
               $(".invoice-validation-err").text("Invoice no is alreay exist against selected vendor !");
