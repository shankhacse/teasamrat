...
$(document).on('change','.igst',function(){
  ...
  $("#addnewDtl").click(function(){
      ...
      if(pMastId!=''){
      window.location.href = basepath+'gstpurchaseinvoice/addPurchaseDetails/masterId/'+pMastId+'/cost_trans/'+cost_trans;;
...
function updateMasterArea(){
  ...
  /* var transporterid = $("#transporterid").val();
   ...
   if (checkUpdatePurchaseInvoiceGstExist()) {
...
// add on 06.04.2018 by shankha
function checkUpdatePurchaseInvoiceGstExist(){
 
    var basepath = $("#basepath").val();
   
    var oldpurchaseInvoiceNo = $("#oldtaxinvoice").val();
    var purchaseInvoiceNo = $("#taxinvoice").val();  
    var oldvendor = $("#oldvendor").val();
    var vendor = $("#vendor").val();
   
    /*alert(oldpurchaseInvoiceNo);
    alert(purchaseInvoiceNo);
    alert(oldvendor);
    alert(vendor);*/
   ...
   $(".purchase-invoice-validation-err").text('');
    if (oldpurchaseInvoiceNo!=purchaseInvoiceNo || oldvendor!=vendor) {
        $.ajax({ 
            type: "POST",
            url: basepath + "purchaseinvoice/checkExistPurchaseInvoice",
            data: {purchaseInvoiceNo:purchaseInvoiceNo,vendor:vendor},
            dataType: 'html',
            success: function(data) {
              ...
              isValid=false;
              $("#taxinvoice").focus();
               $(".updatepurchase-invoice-validation-err").text("Invoice no is alreay exist against selected vendor !");   
                 }else{
                   //alert("valid data");  
                   isValid=true;  
                 }
