...
// save data on add purchase [02/09/2015]
    $('#savePurchase').click(function() {
      ...
      if (checkPurchaseInvoiceGstExist()) {
...
*/
// add on 05.04.2018 by shankha
function checkPurchaseInvoiceGstExist(){
   ...
   $(".purchase-invoice-validation-err").text('');
        $.ajax({ 
            ...
            success: function(data) {
              ...
              isValid=false;
              $("#taxinvoice").focus();
               $(".purchase-invoice-validation-err").text("Invoice no is alreay exist against selected vendor !");   
                 }else{
                   //alert("valid");  
                   isValid=true;  
                 }
