$(document).ready(function() {
    ...
    $(document).on('keyup', "#voucherDate", function() {
        $("#chqDate").val($(this).val()); 
      });
    ...
    if(getVoucherMasterValidation()){
        if(detailvalidation()){
           ...
           $.ajax({
            ...
            success: function(data) {
               $("#generalVoucher").css("display","block");
              
                if(data.code==1){
                    $("#msg_gv").html("");
                    $("#msg_gv").html(data.msg);
                    var voucher_mst_id=data.voucherMasterId;
                   // alert(voucher_mst_id);
                    $('#printpdf').attr('href',basepath +'generalvoucher/print_item/voucherId/' + voucher_mst_id);
...
function getTotalDebit(){
 ...
 function deleterecord(id){
  ...
  function redirectMe(basepath,redirectto)
    {
