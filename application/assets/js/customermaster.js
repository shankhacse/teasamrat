...
$(function() {
    ...
    $("#save").click(function(e) {
        ...
        {
            err = 1;
            $("#state").addClass("glowing-border");
        }
        if(customer!="" && state!="0" && $("#custExist").val() == "Y")
        {
            var conf =  confirm(customer+" is already exist in this company.\nDo you want to create new one with the same name.");
            if(!conf)
            {
                err = 1;
                return false;
    ...
    //auto complete
    $( "#name" ).autocomplete({
      source: basepath+"customermaster/getCustomerList",
      minLength: 1,
      delay: 10,
    ...
    $("#name").blur(function(){
        ...
        $.ajax({
                type: "POST",
                url: basepath + "customermaster/nameExist",
                data: {cName: custName},
                success: function(data)
                {
                    if (data == 1)
