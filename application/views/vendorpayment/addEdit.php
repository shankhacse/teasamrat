...
<form action="#" name="vendorPaymentForm" id="vendorPaymentForm">
<section id="loginBox" style="height: 300px;">
  ...
  <input type="hidden" id="voucherId" name="voucherId" value="<?php echo($bodycontent['vendorpayment']['voucherId']); ?>"/>
    ...
    <td>Date&nbsp;</td>
    <td><input type="text" name="paymentdate" id="paymentdate" class="textStyle datepicker" value="<?php
    if($header['mode']=='Edit'){echo($bodycontent['vendorpayment']['dateofpayment']); }else{echo date('d-m-Y'); }
      ?>" readonly /></td>
  ...
  <tr>
    <td>Cash/Bank.&nbsp;</td>
    <td>
        <select id="cashorbank" name="cashorbank" class="cashorbank selectStyle">
  ...
  <tr>
    ...
    <td>
      <?php if($header['mode']=="Edit"){?>
        <input type="hidden" name="hidenChqno" id="hidenChqno" class="textStyle chqdt" value="<?php echo($bodycontent['vendorpayment']['cheque_number']); ?>" />

        <input type="hidden" name="hidenChqdate" id="hidenChqdate" class="textStyle chqdt" value="<?php echo($bodycontent['vendorpayment']['cheque_date']); ?>" />
        <input type="text" name="chequedt" id="chequedt" class="textStyle chqdt" value="<?php echo($bodycontent['vendorpayment']['cheque_date']); ?>" />
      <?php }else{ ?>
        <input type="text" name="chequedt" id="chequedt" class="textStyle chqdt" value="<?php echo($bodycontent['vendorpayment']['cheque_date']); ?>" />
      <?php } ?>
    </td>
    ...
    <table >
        <tr>
            <td style="padding:0 15px 0 15px;">
                <div id="billList" class="billLists">
      ...
      foreach ($bodycontent["vendorPaymentDtl"] as $value) {
            ?>
              ...
              <td><?php echo($value["billNo"]); ?>
                <input type='hidden' name='vendorBillMstId' value='<?php echo($value["vendorBillMaster"]); ?>'/>
                <input type='hidden' name='vendBillMasterIDS[]' value='<?php echo($value["vendorBillMaster"]); ?>'/></td>
              </td>
              ...
              data-vendorbillmastid="<?php echo($value["vendorBillMaster"]); ?>" 
              data-vendorpaymentdtlid="<?php echo($value["vendorpaymentDtlId"]); ?>" 
              class='rowDel' style='cursor: pointer;' /></td>
              </tr>
