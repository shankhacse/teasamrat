...
<form id="frmCompany" name="frmCompany" method="post" action="<?php echo base_url(); ?>companymaster/updateData">
    <input type="hidden" name="company_hiden_id" value="<?php echo $bodycontent['company_data']->id; ?>" readonly >
  ...
  <tr>
    ...
    <td><select name="state" id="state" style="width: 190px">
        <?php foreach ($bodycontent['states'] as $content) : ?>
             <option value="<?php echo $content->id; ?>" <?php if($bodycontent['company_data']->state_id==$content->id){echo "selected"; }else{echo "";} ?>><?php echo $content->state_name; ?></option>
        <?php endforeach; ?>
        </select>
    </td>    
  ...
  <tr>
  			<td colspan="5">
            	<span class="buttondiv">
          				<div class="save" id="addnew" align="center">Save</div>
      			</span>
            </td>
  </tr>
</table>
