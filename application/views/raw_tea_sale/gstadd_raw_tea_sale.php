...
.gardenTeaSaleDiv input
{
  height:23px;
}
.highlight-gardensale
{
...
<table class="masterTable" width="80%" align="center">
  <tr>
   ...
   <tr>
       ...
       <td width="20%">
           <input type="text" name="invoice_no" id="invoice_no" class="invoice_no" 
                  value="<?php echo $bodycontent['rawteasaleMastData']['invoice_no']?>" style="width:240px;height: 25px;" readonly="readonly" />
       </td>
       ...
       <td width="20%">
        ...
        if($header['mode']=="Add"){
        ?>
          <input type="text" class="datepicker" name="saleDt" id="saleDt" 
                              value="<?php echo $entryDt; ?>"/>
        <?php }else{ ?>
          <input type="text" class="datepicker" name="saleDt" id="saleDt" 
   ...
   <tr>
      <td width="10%">Customer</td>
        <td width="20%">
            <select name="customer" id="customer" class='custom-select' onchange="getConsignee(this.value);"> 
  ...
  <tr>
      ...
      <td>
               <select name="transporterid" id="transporterid" style="width: 245px;height: 25px;">
                        <option value="0">Select</option>
                        <?php foreach ($header['transporterlist'] as $rows) : ?>
                            <option value="<?php echo $rows->id; ?>"  

                            <?php if($header['mode']=="Edit"){ 
                            if($bodycontent['rawteasaleMastData']['transporter_id']==$rows->id){echo('selected');}else{echo('');} 
                              }?>
                              >
    ...
    <tr>
      ...
      <td width="20%"><input name="txtHSN" id="txtHSN" style="width:240px;height: 25px;" placeholder="Harmonized System Nomenclature" type="text"
        value="<?php if($header['mode']=="Edit"){ echo $bodycontent['rawteasaleMastData']['hsn'];
        }?>"></td>
      <td>Consignee</td>      
     <td id="consignee_td">
                
                    <select id="sel_consignee" name="sel_consignee"  style="width:245px;height: 25px;">
                       <option value="">Select</option>  


                      <?php if ($header['mode'] == "Edit") { ?>
                     
                      <?php foreach ($header['consigneetList'] as $rows) {
                 
                  ?> 
                 
                 <option value="<?php echo $rows->id; ?>"
                  <?php if($bodycontent['rawteasaleMastData']['consignee_id']==$rows->id){echo('selected');}else{echo('');} ?>
...
<section id="loginBox">
    ...
    <table class="CSSTableGenerator" >
        ...
        foreach ($bodycontent['rawteasaleDtlData'] as $rows) {
         ...
         ?>
            <tr class="<?php echo $tr_style; ?>" id="rowdtl_<?php echo($rows['rawteasaledtlId']); ?>">
                <td title="Invoice" >
                ...
                <td title="Bag in Stock" align="right" class="highlight-gardensale">
                ...
                <td title="Delivery Charge">
                    <input type="text" id="txtDelivery_<?php echo($rows['PbagDtlId']); ?>" name="txtDelivery[]" 
                    value="<?php echo($rows['delivery_charge']); ?>" 
                    onkeypress="checkNumeric(this);"
			...
			title="Delete" id="<?php echo($rows['rawteasaledtlId']); ?>" class="del-row"
