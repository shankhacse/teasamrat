   ...
   public function getGeneralLedgerReportType3($frmDate,$toDate,$companyId,$yearid,$accId,$fiscalStartDt)
    {
	  ...
	  public function getGeneralLedgerReportType2($compny,$year,$accId,$frmDate,$toDate,$fiscalSdt){
     
      
         $data = array();
         $call_procedure = "CALL usp_generalLedgerDetailSum($compny,$year,$accId,"."'".$frmDate."'".","."'".$toDate."'".","."'".$fiscalSdt."'".")";
            ...
            foreach ($query->result() as $rows) {
                 $data[] = array(
                       "account"=>$rows->accounts,
                        ...
                        "voucherdate"=>$rows->_voucherdate,
                        "vouchernumber"=>$rows->_vouchernumber,
						"dbt_string"=>$rows->sumdebt,
						"crdt_string"=>$rows->sumcrdt,
                        "accounttag"=>$rows->acctag,
                        "transtype"=>$rows->_trantype,
                        //"debitamt"=>$rows->_debitamount,
                        //"creditamt"=>$rows->_creditamount,
						"totalDebit" => $rows->grandtotalDebt,
						"totalCredit" => $rows->grandtotalCredit
