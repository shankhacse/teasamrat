 ...
 */
  ...
  */
   ...
   // modified on 06.04.2018 by shankha
    public function checkExistingInvoice($purchaseInvoice,$vendor,$year){

     $where = array(
       "purchase_invoice_master.`purchase_invoice_number`"=>$purchaseInvoice,
       "purchase_invoice_master.`vendor_id`"=>$vendor,
       "purchase_invoice_master.`year_id`"=>$year
        );

         $sql = $this->db->select('id')->from('purchase_invoice_master')->where($where)->get();
        # echo $this->db->last_query();
          if ($sql->num_rows() > 0) {
 ...
 /**
   ...
   public function GSTinsertintoPurchaseMaster($searcharray,$Vouchrmastid){
      ...
      $pMaster=array();
       ...
       if($searcharray['purchasetype']!='AS'){
             ...
             if (isset($searcharray['challanDate'])) {

                $challan_date=$searcharray['challanDate'];
             }else{
               $challan_date=""; 
             }
            $pMaster['challan_date']=($challan_date == "" ? NULL : date("Y-m-d", strtotime($challan_date))); 

            if (isset($searcharray['challanNo'])) { $challan_no=$searcharray['challanNo'];}else{ $challan_no="";  }
            $pMaster['challan_no']= htmlspecialchars(trim($challan_no));
            if (isset($searcharray['transporterid'])) {
              $transporter_id=$searcharray['transporterid'];
            }else{
              $transporter_id="";  
            }
            $pMaster['transporter_id']=htmlspecialchars(trim($transporter_id));
        }
        else{
        ...
        $pMaster['purchase_invoice_number']=htmlspecialchars(trim($searcharray['taxinvoice']));
        $pMaster['purchase_invoice_date']=date('Y-m-d',  strtotime($searcharray['taxinvoicedate']));
        $pMaster['transfer_date']= NULL;
        if (isset($searcharray['auctionArea'])) {
           $auctionareaid=$searcharray['auctionArea'];
        }else{
           $auctionareaid=""; 
        }
        $pMaster['auctionareaid']=$auctionareaid;
        ...
        $pMaster['cn_no']=htmlspecialchars(trim($searcharray['cnNo']));
        
        $pMaster['tea_value']=htmlspecialchars(trim($searcharray['txtTeaValue']));
        
        
        $pMaster["GST_totalcgst"]=htmlspecialchars(trim($searcharray["txtCGSTTotal"]));
        $pMaster["GST_totalsgst"]=htmlspecialchars(trim($searcharray["txtSGSTTotal"]));
        $pMaster["GST_totaligst"]= htmlspecialchars(trim($searcharray["txtIGSTTotal"]));
        $pMaster["GST_gstincldamt"]=htmlspecialchars(trim($searcharray["txtGSTIncludedAmount"]));
        $pMaster["GST_HSN"]=htmlspecialchars(trim($searcharray["txtHSN"]));
        $pMaster["isGST"]='Y';
        $pMaster["total_bags"]=htmlspecialchars(trim($searcharray["txtTotalBags"]));
        $pMaster["total_kgs"]=htmlspecialchars(trim($searcharray["txtGrandWeight"]));
        

        
        
        $pMaster['round_off'] = htmlspecialchars(trim($searcharray['txtRoundOff']));
        $pMaster['total']=htmlspecialchars(trim($searcharray['txtTotalPurchase']));//total purchase value
  ...
  public function GSTpDetailsInsert($insertPId,$searcharray){
      ...
      for($i=0;$i<$countDtl;$i++){
            ...
            if(isset($searcharray['drplocation'][$i]))
            {
                $detailsData['location_id'] = $searcharray['drplocation'][$i];
            }
            else
            {
               $detailsData['location_id'] = NULL;
            }
   ...
   /**
     ...
     * @date 17.02.2018
     * @return boolean
     * @description Insert New Purchase Invoice Single
     */
    public function GSTinsertSinglePurchaseDetail($pDtl, $bagDtl, $normalBagData,$voucherdata) {
		$masterDataforupdt = array();
        $BagData = array();
        try {
            $this->db->trans_begin();
            //detail
			$this->db->insert('purchase_invoice_detail', $pDtl);
            ...
            $i = 0;
            $noOfBags = count($bagDtl['sampleBag']['txtNumOfSampleBag']);
			
			 for($i = 0; $i < $noOfBags; $i++) {

                $BagData['purchasedtlid'] = $purchaseDetailId;
                $BagData['bagtypeid'] = 2;
                $BagData['no_of_bags'] = $bagDtl['sampleBag']['txtNumOfSampleBag'][$i];
                $BagData['net'] = $bagDtl['sampleBag']['txtNumOfSampleNet'][$i];
                $BagData['chestSerial'] = $bagDtl['sampleBag']['txtNumOfSampleChess'][$i];
                $BagData['actual_bags'] = $bagDtl['sampleBag']['txtNumOfSampleBag'][$i];
                if ($bagDtl['sampleBag']['txtNumOfSampleBag'][$i] != 0) {
                    $this->db->insert('purchase_bag_details', $BagData);
                }
            }
   ...
   /**
    ...
    public function GSTgetPurchaseDataDtls($masterId) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data['GST_totalsgst'] = $rows->GST_totalsgst; 
...
/**
  ...
  public function pBagInsert($dtlId,$dtlNumber,$searcharray,$indexof_i){
     ...
     if(isset($searcharray[$numberofsamplebagIndex]))
     {
...
// added on 09.06.2018
  public function getFiscalStartDt($yearid){
    ...
    public function getFiscalEndDt($yearid){
        $sql="SELECT end_date FROM financialyear WHERE financialyear.id=".$yearid;
        $query = $this->db->query($sql);
         if ($query->num_rows() > 0) {
                foreach ($query->result() as $rows) {
