...
class finishedproductmodel extends CI_Model {
	...
	public function getBlendingListByComp($comp,$year) 
	{
		$data = array();
        $sql ="CALL sp_GETBlendingList(".$comp.",".$year.")";
	...
	/*
	public function getBlendingListByComp($comp) 
	{
        $data = array();
        $sql = "SELECT blending_master.id,
				blending_master.blending_ref
                FROM blending_master
				WHERE blending_master.companyid=".$comp ." ORDER BY blending_master.id ";
     ...
     */
     public function getBlendedKgs($id) {
        ...
        if ($query->num_rows() > 0) 
		{
           ...
           // $consumedkgs = (($row->consumed==NULL or $row->consumed=="")?0:$row->consumed);
