...
class gstservicepurchasemodel extends CI_Model {
   ...
   */
   ...
   ///============================================================================///
    ...
    /*
    @method : deleteServicePurchaseGST
    @date : 27.03.2018
    @by : Mithilesh
    @Table For Del : vendorbillmaster,rawmaterial_purchasedetail,rawmaterial_purchase_master,voucher_detail,voucher_master
    */

    public function deleteServicePurchaseGST($purchaseID,$insertArry)
    {
        $voucherID = $this->getVoucherMasterIDServicePurchase($purchaseID);

        try 
        {
            $this->db->trans_begin();

            // delete vendorbillmaster
            $venbillmaster_whr = array(
                "vendorbillmaster.invoiceMasterId" => $purchaseID,
                "vendorbillmaster.purchaseType" => 'O'
            );

            $this->db->where($venbillmaster_whr);
            $this->db->delete('vendorbillmaster'); 
               
            // delete rawmaterial_purchasedetail
            $this->db->where('rawmaterial_purchasedetail.rawmat_purchase_masterId', $purchaseID);
            $this->db->delete('rawmaterial_purchasedetail');    

            // delete rawmaterial_purchase_master
            $this->db->where('rawmaterial_purchase_master.id', $purchaseID);
            $this->db->delete('rawmaterial_purchase_master');    
    ...
    private function getVoucherMasterIDServicePurchase($purchaseID)
    {
        $voucherID =0;
        $sql = "SELECT rawmaterial_purchase_master.`voucher_id` FROM rawmaterial_purchase_master 
                WHERE rawmaterial_purchase_master.id=".$purchaseID." AND  rawmaterial_purchase_master.IsService='Y'";
        $query = $this->db->query($sql);
        if($query->num_rows()>0)
        {
            $row = $query->row();
