...
class customeradvanceadjstmentmodel extends CI_Model{
     ...
     public function getCustomerAdvanceVoucher($customerAccId){
        ...
        $sql="SELECT `customeradvance`.`advanceId`,
            ...
            WHERE 
            `customeradvance`.`payment_type`='RC'
            AND `customeradvance`.`customeraccountid`=".$customerAccId;
     ...
     public function getSaleInvoiceByCustomer($custAccid,$invalue=NULL){
        $data="";
        $session = sessiondata_method();
        $companyId = $session['company'];

         /* close on 19.05.2018 by shankha
          $sql="SELECT 
        ...
        if(strlen($invalue)>0)
        {
             $sql = "CALL SP_GetCustomerUnpaidBillListRePop (" . $custAccid . "," . $companyId . ",'".$invalue."')";
           $this->db->freeDBResource($this->db->conn_id); 
        }   
        else{
               $sql = "CALL SP_GetCustomerUnpaidBillList (" . $custAccid . "," . $companyId . ")";
            $this->db->freeDBResource($this->db->conn_id); 
        }
...
WHERE customeradvanceadjstdtl.`custadjmstid` = " . $custadjustmentId;
     ...
     public function getCustAdjstDtl($advanceMasterId) {
      ...
      public function delete($customerAdvanceAdjustment,$insertArry) {
        try {
            $this->db->trans_begin();
