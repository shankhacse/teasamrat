...
class vendoradvanceadjstmodel extends CI_Model {
    ...
    public function getVendorAdvanceVoucher($vendorId) {
        ...
        $sql = "SELECT `vendoradvancemaster`.`advanceId`,
            ...
            WHERE 
            `vendoradvancemaster`.`payment_type`='PY'
            AND `vendoradvancemaster`.`vendorId`=" . $vendorId;
    ...
    public function getPurchaseInvoiceByVendor($vendorId,$invalue=NULL) {
        $data = array();
        ...
        if(strlen($invalue)>0)
        {
           $sql = "CALL SP_GetVendorUnpaidBillListRePop (" . $vendorId . "," . $companyId . ",'".$invalue."')";
           $this->db->freeDBResource($this->db->conn_id); 
        }   
        else{
            $sql = "CALL SP_GetVendorUnpaidBillList (" . $vendorId . "," . $companyId . ")";
            $this->db->freeDBResource($this->db->conn_id); 
        }
