...
class vendorpaymentmodel extends CI_Model {
    ...
    private function insertVoucherMaster($masterData) {
        ...
        $voucherMasterArray["transaction_type"] = $masterData["transaction_type"]; // vendor advance
...
private function updateVoucherMaster($masterData){
    ...
    public function getVendorPaymentList($compny,$year){
        ...
        $sql= "SELECT
                ...
                WHERE voucher_master.transaction_type <> 'PJV' AND
                voucher_master.`company_id`=".$compny." AND voucher_master.year_id=".$year;
    ...
    public function getVendorOtherAdjPaymentList($compny,$year){
        ...
        $sql= "SELECT
                ...
                `voucher_master`  ON `vendorbillpaymentmaster`.`voucherId` = `voucher_master`.`id`
                INNER JOIN vendor ON vendorbillpaymentmaster.`vendorid` = vendor.`account_master_id`
                WHERE voucher_master.transaction_type='PJV'
                AND voucher_master.`company_id`=".$compny." AND voucher_master.year_id=".$year;
        ...
        $sql="SELECT 
                  ...
                  (
                    CASE
                      ...
                      WHEN purchase_finishproductmaster.`id` IS NOT NULL
                      THEN purchase_finishproductmaster.`purchasebillno`
                    END
                  ) AS billNo,
                  (
                    CASE
                  ...
                  (
                    CASE
                      ...
                      WHEN purchase_finishproductmaster.`id` IS NOT NULL
                      THEN purchase_finishproductmaster.`grandtotal`
                    END
                  ) AS BillAmount 
                FROM
                  `vendorbillpaymentdetail` 
                  INNER JOIN `vendorbillmaster` 
                    ON vendorbillpaymentdetail.`vendorBillMaster` = vendorbillmaster.`vendorBillMasterId` 
                  ...
                  LEFT JOIN `purchase_finishproductmaster`
                    ON vendorbillmaster.`invoiceMasterId` = `purchase_finishproductmaster`.`id` 
                    AND vendorbillmaster.`purchaseType` = 'F' 
                  
                WHERE vendorbillpaymentdetail.`vendorpaymentid` = ".$vendorpaymentId;
    ...
    */
    ...
    public function DeleteVendorPayment($vendorPaymentID,$insertArry)
    {
        $voucherID = $this->getVoucherMasterIDFromVendorPayment($vendorPaymentID);
       
        try 
        {
            $this->db->trans_begin();
               
            // delete vendorbillpaymentdetail
            $this->db->where('vendorpaymentid', $vendorPaymentID);
            $this->db->delete('vendorbillpaymentdetail'); 
            
            // delete vendorbillpaymentmaster
            $this->db->where('vendorPaymentId', $vendorPaymentID);
            $this->db->delete('vendorbillpaymentmaster'); 
    ...
    //
    private function getVoucherMasterIDFromVendorPayment($vendorPaymentID)
    {
        $voucherID =0;
         $sql = "SELECT  vendorbillpaymentmaster.`voucherId`  FROM `vendorbillpaymentmaster` 
                WHERE `vendorbillpaymentmaster`.`vendorPaymentId`=".$vendorPaymentID;
    ...
    public function deleteVendorPaymentDtl($vendorBillMasterId,$vendorpaymentdtlId)
    {

        $where = array(
            
            "vendorbillpaymentdetail.`vendorBillMaster`"=>$vendorBillMasterId,
            "vendorbillpaymentdetail.`id`"=>$vendorpaymentdtlId
        );
       try 
        {
            $this->db->trans_begin();
