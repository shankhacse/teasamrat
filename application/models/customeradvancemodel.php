...
class customeradvancemodel extends CI_Model {
 ...
 private function getSerialNumber($company,$year){
    ...
    private function InsertVoucherDetail($voucherDetailArray) {
        ...
        if ($voucherDetailArray["payment_type"]=="RC") {
 ...
 */   
    ...
    public function getCustomerAdvanceById($customeradvanceId) {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
                    ...
                    "account_name" => $rows->account_name,
                    "voucher_number" => $rows->voucher_number,
                     "cashorbankId"=>  $this->getDebitAccountId($rows->voucherid,$rows->payment_type));
            }
     ...
     public function getDebitAccountId($voucherId,$payment_type){
	...
	public function DeleteCustomerAdvance($cusAdvanceID,$insertArry)
	{
...
public function checkCustomerAdjustmentExistanceData($customeradvanceId)
  {
        $where = array(
