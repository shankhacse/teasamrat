...
class doreceivedmodel extends CI_Model{
    ...
    public function StockInByTransporter($transporterId='',$company,$yearid,$fromChallanDate,$toChallanDate){
             ...
             $sql = "SELECT 
                            ...
                            `garden_master`.`garden_name`,
                              blending_details.`id` AS blendingDtlID,
                            `rawteasale_detail`.`id` AS rawteasaleID,
                            `stocktransfer_out_detail`.`id` AS stocktransferOutID 
                          FROM
                            `do_to_transporter` 
                            ...
                            LEFT JOIN `location` 
                              ...
                              ON blending_details.`purchase_dtl_id` = purchase_invoice_detail.`id`
                              
                              LEFT JOIN `rawteasale_detail`
                              ON `rawteasale_detail`.`purchase_detail_id` = purchase_invoice_detail.`id`
                              
                              LEFT JOIN `stocktransfer_out_detail`
                              ON `stocktransfer_out_detail`.`purchase_detail_id` = purchase_invoice_detail.`id`

                            WHERE `do_to_transporter`.`transporterId` =".$transporterId." AND do_to_transporter.in_Stock='Y' 
                             AND (do_to_transporter.chalanDate BETWEEN '".$fromChallanDate."' AND '".$toChallanDate."')    

                            AND blending_details.`id` IS NULL
                            AND `rawteasale_detail`.`id` IS NULL
                            AND `stocktransfer_out_detail`.`id` IS NULL
