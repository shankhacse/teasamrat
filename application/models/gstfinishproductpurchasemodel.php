...
class gstfinishproductpurchasemodel extends CI_Model{
    ...
    public function getFinishProdMasterData($purchaseBillId){
       $sql="SELECT
			...
			purchase_finishproductmaster.GST_FreightamountNew,
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
					...
					"GST_FreightAmt"=> $rows->GST_FreightamountNew
    ...
    public function getFinishProdDetailData($pBillid){
        $sql="SELECT
                ...
                purchase_finishproductdetail.freightamount,
