...
class rawteasalemodel extends CI_Model {
    ...
    public function getGSTRawTeasaleList($cmpny,$year){
        ...
        $sql ="SELECT 
                ...
                INNER JOIN purchase_invoice_master ON PID.purchase_master_id = purchase_invoice_master.id
...
public function getBlendedBag($bagDtlId){
   ...
   private  function updateBillMaster($taxinvoiceId,$searcharray){
    ...
    public function GSTupdateRawTeaSaleMaster($searcharray){
        ...
        if (isset($searcharray['sel_consignee'])) {
                if ($searcharray['sel_consignee']=="") {
                    $rawTeasalemaster['consignee_id']=NULL;
                }else{
                     $rawTeasalemaster['consignee_id']=$searcharray['sel_consignee'];
                }
            }
...
/*@method insertintoVouchrDtl
    ...
    public function GSTinsertintoRawTeaSaleMaster($vMastId,$searcharray,$rawTeaSaleInvoice){
        ...
        $rawteasale['isGST']='Y';
         ...
         if (isset($searcharray['sel_consignee'])) {
                if ($searcharray['sel_consignee']=="") {
                    $rawteasale['consignee_id']=NULL;
                }else{
                     $rawteasale['consignee_id']=$searcharray['sel_consignee'];
                }
            }
   ...
   /* public function getCurrentvatrate($startYear, $endYear) {
    ...
    public function GSTRawTeaSalemasterData($rawteaslMastId){
        $sql = "SELECT 
                ...
                rawteasale_master.consignee_id
              FROM
                rawteasale_master 
        ...
        if($query->num_rows() >0){
            foreach ($query->result() as $rows){
                $data= array(
                    ...
                    "consignee_id" => $rows->consignee_id
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
                    ...
                    "bank_branch_add"=>$rows->bank_branch_add,
                );
            }
