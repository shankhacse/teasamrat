 ...
 /*           
    ...
    public function saleNumberList($cmpyid,$yearid){
       $sql="SELECT DISTINCT `purchase_invoice_master`.`sale_number` FROM `purchase_invoice_master` 
                  WHERE `purchase_invoice_master`.`sale_number`<>'' AND purchase_invoice_master.`company_id`=".$cmpyid." AND purchase_invoice_master.`year_id`=".$yearid." 
                ORDER BY CONVERT(`purchase_invoice_master`.`sale_number`,DECIMAL) ASC";
