...
class salebillregistermodel extends CI_Model {
   ...
   function getCustomerList($session_data)
	{
       ...
       $this->db->join('account_opening_master', 'account_master.id = account_opening_master.account_master_id AND account_opening_master.financialyear_id='.$session_data['yearid'].'','LEFT');
   ...
   /**
	...
	public function getSaleBillRegisterData($fdate,$tdate,$cusid,$compnyId,$yid,$isGST,$detail_order_by)
	{
		$data = array();
		$sql = "CALL sp_SaleBillRegister('".$fdate."','".$tdate."',".$cusid.",".$compnyId.",".$yid.",'".$isGST."','".$detail_order_by."')";
		//echo $sql;
    ...
    public function getSaleBillRegisterDataInvoiceGstDtl($fdate,$tdate,$in_customer,$in_product,$compnyId,$yid,$isGST)
    {
        $data = array();
        $sql = "CALL sp_SaleBillRegisterInvoiceDtl('".$fdate."','".$tdate."','".$in_customer."','".$in_product."',".$compnyId.",".$yid.",'".$isGST."')";
        //echo $sql;
        $query = $this->db->query($sql);
...
public function getSaleBillRegisterDataGardenTeaGstDtl($fdate,$tdate,$in_customer,$in_group_code,$compnyId,$yid,$isGST)
    {
        $data = array();
        $sql = "CALL sp_SaleBillRegisterGardenTeaDtl('".$fdate."','".$tdate."','".$in_customer."','".$in_group_code."',".$compnyId.",".$yid.",'".$isGST."')";
        //echo $sql;
        $query = $this->db->query($sql);
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                    ...
                    "sgstamount"=>$rows->sgstamount,
                    "igstamount"=>$rows->igstamount,
                    "teagroupId"=>$rows->teagroupId,
                    "groupcode"=>$rows->teagroupcode,
                    "customerid"=>$rows->customerid,
                    "rate"=>$rows->rate,
                    "gardenname"=>$rows->gardenname,
                    "detailinvoice"=>$rows->detailinvoice
                );
        }
