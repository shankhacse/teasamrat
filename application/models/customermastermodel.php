   ...
   public function custoemrlist($session_data)
	{
	  ...
	  // echo $this->db->last_query();
	   ...
	   else
	   {
        ...
        public function getAutoCompltCustomer($term,$comp)
        {
             $data =array();
             ...
             $this->db->like('customer_name', $term, 'after');
    ...
    public function getCustomerExist($name,$cmpny)
    {
            
        $this->db->select('customer.id as vid,customer.customer_name');
        $this->db->from('customer');
        $this->db->join('account_master', 'customer.account_master_id = account_master.id','LEFT');
        $this->db->where('customer.customer_name', $name);
	...
	public function modify($value)
	{
		...
		if($value['opening_balance'] != '')
		{
			...
			// delete from account_opening_master where accid and financial_year is considered
			 $whereacopmst = array(
                    'account_opening_master.account_master_id' => $value['account_master_id'] ,
                    'account_opening_master.financialyear_id' => $session['yearid'] 
                );
				
				
			 $this->db->where($whereacopmst);
			 $this->db->delete('account_opening_master'); 
						
			// then insert new record with accid and financial_year
...
WHERE `vendor`.`id` = 2*/	
	   ...
	   $this -> db -> join('`account_master`', 'customer.account_master_id = account_master.id','left');
	   $this->db->join('account_opening_master', 
	   'account_master.id = account_opening_master.account_master_id AND account_opening_master.financialyear_id ='.$session['yearid'].' ',
	   'LEFT');
	   //$this -> db -> where('account_opening_master.financialyear_id',$session['yearid']);
