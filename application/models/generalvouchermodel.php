       ...
       try {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                 ...
                 $result=array(
                    "code"=>1,
                    "msg"=>"Data successfully saved with Voucher No : ".$insertVoucherMaster['voucher_number'],
          ...
          /****/
			if($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                 ...
                 $result=array(
                    "code"=>1,"msg"=>"Data successfully Updated.",
                    "voucherMasterId"=>$vouchrMastId
                );
                return $result;
            }
			/****/
...
public function getLastSerialNo($cid,$yid)
 ...
 public function deleteGeneralVoucher($voucherMastId)
    ...
    ===========================================================================
    */
     public function getAccountByGroupMasterWithoutSundryDebAndCreditor($companyId){
       $sql="SELECT `account_master`.`account_name`,
            `account_master`.`id` AS accountId,
            `group_master`.`id` AS groupmasterId,
            `group_master`.`group_name`
            ...
            INNER JOIN `group_master`
            ON `group_master`.`id`=`account_master`.`group_master_id`
            WHERE `group_master`.`group_name` NOT IN('Sundry Debtors','Sundry Creditors')
            AND account_master.`company_id` =".$companyId." ORDER BY `account_master`.`account_name`";
