 ...
 function index()
 {
	...
	else
   {
	...
	function delete()
	{
		...
		$this->customermastermodel->delete($value);
	}
	...
	*/
	function dispalyEditDetails()
	{
		...
		if($data != '')
		{
			...
			{
				$row .= '<tr><td>'.$value->tax_invoice_number.'</td><td>'.$value->tax_invoice_date.'</td><td>'.$value->bill_amount.'</td><td>'.number_format($value->due_amount, 2, '.', '').'</td></tr>';	
			}
		}
		else
		{
