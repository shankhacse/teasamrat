...
class Purchaseinvoice extends CI_Controller {
 ...
 function index()
 {
	...
	else
   {
	...
	function savedata()
	{
		$value['tax_invoice_number'] =  $this->input->post('taxinvoice');
		$value['sale_number'] =  $this->input->post('salenumber');
		$value['tax_invoice_date'] =  date("Y-m-d", strtotime($this->input->post('taxinvoicedate')));
		...
		for($i= 0; $i<$count; $i++)
		{
			...
			$value_item['purchase_invoice_id'] = $status;
			...
			$arrsample = explode('<br/>',$samplenamearr[$i]);
			$arrsamplenet = explode('<br/>',$samplenetarr[$i]);
	...
	function getlistingindetail()
	{
		 ...
		 $result = $this->purchaseinvoicemastermodel->getPurchaselistingdetaildata($invoiceid);
	...
	function update()
	{
		...
		for($i= 0; $i<$count; $i++)
		{
		 ...
		 $status4 = $this->purchaseinvoicemastermodel->updatedateVoucherMaster($value['tax_invoice_date'],$this->input->post('voucherid'));
