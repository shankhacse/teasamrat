        ...
        if ($this->session->userdata('logged_in')) {
            ...
            */
            ...
            if ($salebillno != 0) {
                ...
                $customerId=$result['taxInvoiceMaster']['customerid'];
      ...
      //  echo($saleRate);
        echo json_encode(
                array(
                    ...
                    "hsn_code" =>$saleRate['hsn_code'],
                )
                );
    }


    public function delSaleBillGST()
    {
        ...
        if ($this->session->userdata('logged_in')) 
        {
              ...
              $salebillno = $this->input->post('salebillno');
                ...
                $checkcustajustment = $this->checkdatamodel->checkCustomerAdjustmentAgainstBill($salebillID,'T');
                $checkcustomerreceipt = $this->checkdatamodel->checkCustomerReceiptAgainstBill($salebillID,'T');
                ...
                else
                {
                   ...
                   $user_activity_data =  array(
                                    ...
                                    "activity_module" => "Sale Bill GST",
                                    "action" => "Delete",
                                    "narration" => $salebillno." deleted ID is ".$salebillID,
                                    "from_method" => "GSTtaxinvoice/delSaleBillGST",
                                    "user_id" => $session['user_id']
                                 );
                  

                  $delete = $this->gsttaxinvoicemodel->deleteSaleBillTaxInvoiceGST($salebillID,$user_activity_data);
                  if($delete)
                  {
                      $json_response = array(
                          "ERROR"=>0,
