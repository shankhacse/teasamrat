...
class vendorpaymentotheradj extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('logged_in')) {
            $result = $this->vendorpaymentmodel->getVendorOtherAdjPaymentList($session['company'],$session['yearid']);
            $page = 'vendorpayment/list_viewOtherAdj';
            $header = '';
            $headercontent = '';
    ...
    public function addVendorPayment() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $headercontent['CashOrBank'] = $this->generalvouchermodel->getAccountByGroupMasterWithoutSundryDebAndCreditor($session['company']);

            if ($vendorpaymentId == 0) {
                $headercontent['mode'] = "Add";
                $page = 'vendorpayment/addEditOtherAdj';
                $result = "";
            } else {
    ...
    public function saveVendorPayment() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $masterData["paymentType"] = $this->input->post("paymentType");

            $masterData["transaction_type"] = 'PJV';
