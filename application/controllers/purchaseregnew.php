...
public function getGSTPurchaseRegister()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$result['forperiod'] = $fromDate." To ".$toDate;
                        $result['purchaseRegister']=$this->purchaseregistermodel->generateGstPurchaseRegister($compnyID,$yearID,$fDate,$toDt,$in_vendor);
			...
			if(strlen($in_vendor)>0)
			{
				$page = "purchase_register/purchase_register_new/purchase_register_vendorwise_pdf_gst";
			}
			else
			{
