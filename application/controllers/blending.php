...
//        echo('</pre>');
  ...
  */
    ...
    public function blendingDetails() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $yearId =$session['yearid'];
             ...
             $fiscalDt = $this->blendingmodel->getFiscalStartDt($yearId);
             $result['fiscalStartDt'] = $fiscalDt->start_date;
             $result['fiscalEndtDt'] = $fiscalDt->end_date;
         
            $page = 'blending/header_view.php';
            $header = '';
            $headercontent="";
   ...
   //------------------------------------------------------ 
   public function getblendingRegisterPdf(){
        $session = sessiondata_method();
       ...
       $result['resultBlending'] = $this->blendingmodel->blendingRegByDate($fdate,$tdate,$companyId,$yearId);

          /*  echo "<pre>";
            print_r($result['resultBlending']);
            echo "</pre>";
             exit;*/
              $page = 'blending/blending_register_pdf.php';
         ...
         $html = $this->load->view($page, $result, TRUE);
                $pdf->WriteHTML($html); 
                $output = 'blending' . date('Y_m_d_H_i_s') . '_.pdf'; 
                $pdf->Output("$output", 'I');
                exit();
