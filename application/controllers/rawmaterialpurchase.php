...
class rawmaterialpurchase extends CI_Controller {
    ...
    public function GSTinsertData($searcharray) {
        ...
        if ($this->session->userdata('logged_in')) { htmlspecialchars(trim($searcharray['orderno']));
			
            $rawMaterialPurchase['invoice_no'] = htmlspecialchars(trim($searcharray['invoiceno']));
            $rawMaterialPurchase['invoice_date'] = date("Y-m-d", strtotime($searcharray['invoicedate']));
            $rawMaterialPurchase['challan_no'] =htmlspecialchars(trim( $searcharray['challanno']));
            $rawMaterialPurchase['challan_date'] =($searcharray['challandate'])=="" ? NULL : date("Y-m-d", strtotime($searcharray['challandate']));
            $rawMaterialPurchase['order_no'] = htmlspecialchars(trim($searcharray['orderno']));
			...
			*/
			//$rawMaterialPurchase['rateType'] = NULL;
			$rawMaterialPurchase['taxrateTypeId'] = NULL;
     ...
     public function GSTupdateData($rowpurMastId, $searcharray) {
        ...
        if ($this->session->userdata('logged_in')) {
           ...
           $updRowmatPurchaseMast['id'] = $rowpurMastId;
            $updRowmatPurchaseMast['invoice_no'] = htmlspecialchars(trim($searcharray['invoiceno']));
            $updRowmatPurchaseMast['invoice_date'] = date("Y-m-d", strtotime($searcharray['invoicedate']));
            $updRowmatPurchaseMast['challan_no'] = htmlspecialchars(trim( $searcharray['challanno']));
            $updRowmatPurchaseMast['challan_date'] = ($searcharray['challandate'])=="" ? NULL : date("Y-m-d", strtotime($searcharray['challandate']));
            $updRowmatPurchaseMast['order_no'] = $searcharray['orderno'];htmlspecialchars(trim($searcharray['orderno']));
            $updRowmatPurchaseMast['order_date'] = ($searcharray['orderdate'])=="" ? NULL : date("Y-m-d", strtotime($searcharray['orderdate']));

            if (isset($searcharray['exciseinvno'])) {
            $exciseinvno=$searcharray['exciseinvno'];
            }else{ $exciseinvno= "";};

            $updRowmatPurchaseMast['excise_invoice_no'] = $exciseinvno;

            if (isset($searcharray['excisedate'])) {
            $excisedate=$searcharray['excisedate'];
            }else{ $excisedate= "";};

            $updRowmatPurchaseMast['excise_invoice_date'] =$excisedate=="" ? NULL: date("Y-m-d", strtotime($excisedate));
...
//check checkExistPurchaseInvoice

      public function checkInvoiceExistance(){
       // $data="0";
         $session = sessiondata_method();
        if ($this->session->userdata('logged_in')) {

        $invoiceno = htmlspecialchars(trim($this->input->post('invoiceno')));
        $vendor = htmlspecialchars(trim($this->input->post('vendor')));
        $year = $session['yearid'];
        

        
         $result = $this->rawmaterialpurchasemodel->checkInvoiceExistanceData($invoiceno,$vendor,$year);
         if($result==TRUE){
             echo "1";
         }
         else{
