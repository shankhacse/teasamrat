       ...
       $headercontent['transporterlist']=$this->transportmastermodel->transportlist();
             ...
             if ($rawteasaleMastId != 0) {
                ...
                $result['rawteasaleDtlData'] = $this->rawteasalemodel->GSTRawTeaSaleDtlData($rawteasaleMastId);
                 ...
                 $customerId=$result['rawteasaleMastData']['customer_id'];
                
                $headercontent['consigneetList'] = $this->gsttaxinvoicemodel->getConsigneeByCustomerId($customerId,$session['company']);
...
*/
    ...
    public function getpdfRawTeaSale(){
		...
		/*	
			echo "<pre>";
			print_r($result['headerview']);
			echo "</pre>";*/
			
          $consigneeid="";
          $consigneeid=$result['headerview']['consignee_id'];
          if($consigneeid!=""){
          $result['consigndtl'] = $this->gsttaxinvoicemodel->getConsigneeByConsignId($consigneeid);
          }
			//exit;
            $result['amountinword'] = strtoupper( $this->no_to_words($result['headerview']['GrandTotal']));
    ...
    /*
    @method : delRawTeaSaleBillGST
    @date : 27.03.2018
    @BY : Mithilesh
    */

    
    public function delRawTeaSaleBillGST()
    {
        $session = sessiondata_method();
        if ($this->session->userdata('logged_in')) 
        {
              ...
              $salebillID = $this->input->post('salebillID');
              $salebillno = $this->input->post('salebillno');
             
              

                $checkcustajustment = $this->checkdatamodel->checkCustomerAdjustmentAgainstBill($salebillID,'R');
                $checkcustomerreceipt = $this->checkdatamodel->checkCustomerReceiptAgainstBill($salebillID,'R');

                // cheking is sale bill used in other modules or not
                // Module Name : Customer adjustment , Customer Receipt
                if(sizeof($checkcustajustment)>0 || sizeof($checkcustomerreceipt)>0)
                {
                  $custAdjustErr = "";
                  $custReceiptErr = "";

                  if(isset($checkcustajustment) && sizeof($checkcustajustment)>0)
                  {
                    $refno =""; 
                    foreach($checkcustajustment as $custadjustmntData)
                    {
                        $refno.= $custadjustmntData->adjustmentrefno.",";
                    }
                    $refno = rtrim($refno,',');
                    $custAdjustErr = 'Customer Adjustment with Ref No '.$refno;
                  }


                  if(isset($checkcustomerreceipt) && sizeof($checkcustomerreceipt)>0)
                  {
                    $voucher_no = "";
                    foreach($checkcustomerreceipt as $custreceiptData)
                    {
                        $voucher_no.= $custreceiptData->voucher_number.",";
                    }
                     $voucher_no = rtrim($voucher_no,',');
                     $custReceiptErr = 'Customer Receipt with Voucher No '.$voucher_no;
                  }
                  
                  $erroMsgData = array(
                      "textHead" => $salebillno .' is used in following modules : ',
                       "custAdjustErr" => $custAdjustErr,
                       "custReceiptErr" => $custReceiptErr
                   );
                ...
                else
                {
                     ...
                     $user_activity_data =  array(
                                    ...
                                    "activity_module" => "Garden Tea Sale",
                                    "action" => "Delete",
                                    "narration" => $salebillno." deleted ID is ".$salebillID,
                                    "from_method" => "gstrawteasale/delRawTeaSaleBillGST",
                                    "user_id" => $session['user_id']
                                 );

                  $delete = $this->rawteasalemodel->deleteRawTeaSaleBillGST($salebillID,$user_activity_data);
                  if($delete)
                  {
                      $json_response = array(
                          "ERROR"=>0,
                          "MSG" => $salebillno ." successfully deleted"
   ...
   public function getConsignee()
  {
    ...
    if($this->session->userdata('logged_in'))
    {
       ...
       $data['consigneetList'] = $this->gsttaxinvoicemodel->getConsigneeByCustomerId($customer_id,$session['company']);
       
       $viewTemp = $this->load->view('GSTtaxinvoice/consignee_view',$data);
      echo $viewTemp;
    }
    else
    {
