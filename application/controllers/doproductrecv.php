...
public function transporter_stock() {
        $session = sessiondata_method();
        $this->companyId = $session['company'];
        $this->yearId = $session['yearid'];
        $company = $session['company'];
        ...
        $headercontent = '';
        $page = 'do_receive/transporter_report';
        $header = '';
        /* load helper class to create view */
        createbody_method($result, $page, $header, $session, $headercontent);
    }

    public function generateTransStock() {
        ...
        $company = $session['company'];
        $yearid = $session['yearid'];
        $from_date =date('Y-m-d', strtotime($this->input->post('date_form'))) ;
        $to_date = date('Y-m-d', strtotime($this->input->post('date_to'))) ;
        $transporter_id = $this->input->post('transporter');
        
      $result["data"]=$this->doreceivedmodel->StockInByTransporter($transporter_id,$company,$yearid,$from_date,$to_date);
      $result["from"]=$this->input->post('date_form');
      $result["todt"]=$this->input->post('date_to');
      
      
      $result['company']=  $this->companymodel->getCompanyNameById($company);
      $result['companylocation']=  $this->companymodel->getCompanyAddressById($company);
      $result['transporter']= $this->transportmastermodel->getTransporterById($transporter_id);
...
//echo('</pre>');
        ...
        $page = 'do_receive/report_pdf.php';
