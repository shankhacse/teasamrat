 ...
 */
    ...
    public function insertNewPurchaseInvoice(){
        ...
        $voucherMaster = array();
        $voucherMaster['voucher_number']=htmlspecialchars(trim($searcharray['taxinvoice']));
   ...
   /**
    ...
    public function updatePurchaseMasterData() {  
        ...
        $PurchaseMasterId = htmlspecialchars(trim($this->input->post('PurMasterId')));
        $voucherMastId =htmlspecialchars(trim($this->input->post('voucherMastId')));
        $purchaseType = htmlspecialchars(trim($this->input->post('purchaseType')));
        $auctionarea =  htmlspecialchars(trim($this->input->post('auctionarea')));
        $purchaseInvoiceNumber = htmlspecialchars(trim($this->input->post('purchaseInvoiceNumber')));
        $invoiceDate = ($this->input->post('invoiceDate') == "" ? NULL : date("Y-m-d", strtotime($this->input->post('invoiceDate'))));
        $saleNo =  htmlspecialchars(trim($this->input->post('saleNo')));
        ...
        $vendor = htmlspecialchars(trim($this->input->post('vendor')));
        $cnNo =  htmlspecialchars(trim($this->input->post('cnNo')));
        $transporterid =  htmlspecialchars(trim($this->input->post('transporterid')));
        $challanno =  htmlspecialchars(trim($this->input->post('challanno')));
        $challanDt = ($this->input->post('challanDt') == "" ? NULL : date("Y-m-d", strtotime($this->input->post('challanDt'))));
        $HSN =  htmlspecialchars(trim($this->input->post('HSN')));
 ...
 /**
    ...
    public function addPurchaseDetails() 
	{
        ...
        if ($this->session->userdata('logged_in')) {
		   ...
		   $page = 'purchase_invoice/add_purchase_details_single_gst.php';
           $header = '';
	...
	public function addSinglePurchaseInvoice()
	{
		...
		*/
		...
		$purchaseMasterId = $dataArry['txtPmasterId'];
        $voucherMasterId = $dataArry['voucherMastId'];
        $vendorId = $dataArry['vendorId']; 
       #$purchaseDetailId = $dataArry['purchaseDetailId'];
        $lotnumber = $dataArry['txtLot'];
        $doNumber = $dataArry['txtDo'];
        $doDate = ($dataArry['txtDoDate'] == "" ? NULL : date("Y-m-d", strtotime($dataArry['txtDoDate'])));
        $invoiceNumber = $dataArry['txtInvoice'];
        $gross = $dataArry['txtGross'];
        $gpNumber = $dataArry['txtGpnumber'];
        $gpDate = ($dataArry['txtGpDate'] == "" ? NULL : date("Y-m-d", strtotime($dataArry['txtGpDate'])));
        $price = $dataArry['txtPrice'];
        $transCost = $dataArry['transCost'];

        $group = $dataArry['drpgroup'];
        $location = $dataArry['drplocation'];
        $garden = $dataArry['drpGarden'];
        $grade = $dataArry['drpGrade'];
        $wareHouse = $dataArry['drpWarehouse'];

        $normalBagid = $dataArry['txtNormalBagTypeId'];
        $normalBag = $dataArry['txtNumOfNormalBag'];
        $nomalBagQty = $dataArry['txtNumOfNormalNet'];
        $normalBagChest = $dataArry['txtNumOfNormalChess'];
        $sampleBag = $dataArry;
       
        $totalTeaValue = $dataArry['DtltotalValue'];
        $totalWeight = $dataArry['DtltotalWeight'];
        #$totalCost = $dataArry['totalDtlCost'];
        $teaCostPrKg = $dataArry['DtlteaCost'];
        ...
        /****************************GST*******************************/
        $discount=  $dataArry['DtlDiscountValue'];
        $taxable = $dataArry['DtlTaxableValue'];
        $cgstid = $dataArry['cgst'];
        $cgstamt  = $dataArry['cgstAmt'];
        $sgstid = $dataArry['sgst'];
        $sgstamt = $dataArry['sgstAmt'];
        $igstid = $dataArry['igst'];
        $igstamt = $dataArry['igstAmt'];
        $netamount = $dataArry['txtdtlnetamount'];
        /**************************************************************/
        ...
        $data['price'] = $price;
        $data['service_tax'] = NULL;
        $data['total_value'] = $totalTeaValue;
        $data['chest_from'] = NULL;
        $data['chest_to'] = NULL;
        $data['value_cost'] = $totalTeaValue; //[bagsQty*price]
        $data['net'] = null;
        $data['rate_type'] = NULL;
        $data['rate_type_id'] = 0;
        $data['service_tax_id'] = 0;
        $data['teagroup_master_id'] = $group;
        $data['cost_of_tea'] = $teaCostPrKg;
        $data['transportation_cost'] = $transCost;
        /*********************************GST**************************/
        $data['gst_teavalue']=$totalTeaValue;
        $data['gst_discount']=$discount;
        $data['gst_taxable']=$taxable;
        $data['cgst_id']=$cgstid;
        $data['cgst_amt']=$cgstamt;
       ...
       /* echo "<pre>";
        ...
        $insertData = $this->purchaseinvoicemastermodel->GSTinsertSinglePurchaseDetail($data, $bagData, $NormalbagData,$voucherdata);
		if($insertData==TRUE)
		{
			$json_response = array(
				"msg_status" => 1,
