...
class gstservicepurchase extends CI_Controller {
   ...
   /**
    ...
    /*
    @method : delservPurchaseGST
    @date  : 27.03.2018
    @By :  Mithilesh
    */

    public function delservPurchaseGST()
    {
        ...
        {       
              ...
              $purchaseID = $this->input->post('srvPurchID');
              $purchaseno = $this->input->post('invoice');
             
              

                $checkVendorAdjustment = $this->checkdatamodel->checkVendorAdjustmentAgainstBill($purchaseID,'O');
                $checkVendorPayment = $this->checkdatamodel->checkVendorPaymentAgainstBill($purchaseID,'O');

                // cheking is service Purchase used in other modules or not
                // Module Name : Vendor adjustment , Vendor Payment
                if(sizeof($checkVendorAdjustment)>0 || sizeof($checkVendorPayment)>0)
                {
                  $VendAdjustErr = "";
                  $VenPaymentErr = "";

                  if(isset($checkVendorAdjustment) && sizeof($checkVendorAdjustment)>0)
                  {
                    $refno =""; 
                    foreach($checkVendorAdjustment as $venadjustmntData)
                    {
                        $refno.= $venadjustmntData->AdjustmentRefNo.",";
                    }
                    $refno = rtrim($refno,',');
                    $VendAdjustErr = 'Vendor Adjustment with Ref No '.$refno;
                  }


                  if(isset($checkVendorPayment) && sizeof($checkVendorPayment)>0)
                  {
                    $voucher_no = "";
                    foreach($checkVendorPayment as $vendorPaymentData)
                    {
                        $voucher_no.= $vendorPaymentData->voucher_number.",";
                    }
                     $voucher_no = rtrim($voucher_no,',');
                     $VenPaymentErr = 'Vendor Payment with Voucher No '.$voucher_no;
                  }
                  
                  $erroMsgData = array(
                      "textHead" => $purchaseno .' is used in following modules : ',
                       "venAdjustErr" => $VendAdjustErr,
                       "venPaymentErr" => $VenPaymentErr
                   );
                ...
                else
                {
                    ...
                    $user_activity_data =  array(
                                    ...
                                    "activity_module" => "Service Purchase GST",
                                    "action" => "Delete",
                                    "narration" => $purchaseno." deleted ID is ".$purchaseID,
                                    "from_method" => "gstservicepurchase/delservPurchaseGST",
                                    "user_id" => $session['user_id']
                                 );
                  

                  $delete = $this->gstservicepurchasemodel->deleteServicePurchaseGST($purchaseID,$user_activity_data);
                  if($delete)
                  {
                      $json_response = array(
                          "ERROR"=>0,
                          "MSG" => $purchaseno ." successfully deleted"
