...
//added on 19.05.2018
      public function rePopulateSaleBillList() 
    {
        $session = sessiondata_method();
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $customerAccountId = $dataArry['customer'];
            $notinVal ="";
            if(isset($dataArry['custBillMasterIDS']))
            {
                for($i=0;$i<sizeof($dataArry['custBillMasterIDS']);$i++)
                {   
                    
                    $notinVal.=$dataArry['custBillMasterIDS'][$i].",";
                }
            }
            $result['invoices'] = $this->customeradvanceadjstmentmodel->getSaleInvoiceByCustomer($customerAccountId,$notinVal);
      ...
      public function deleteDetailData()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $customerreceiptdtlId = $this->input->post('customerreceiptdtlId');
            

            $delete = $this->customerpaymentmodel->deleteCustomerPaymentDtl($customerreceiptdtlId,$customerBillMasterId);
            if($delete)
            {
   ...
   public function saveCustomerPayment() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $masterData["transaction_type"] = 'RC';
